...
func newPubConsumer(t *testing.T) (*RabbitPublisher, *RabbitConsumer) {
	t.Helper()
	consumer, err := NewRabbitConsumer(testRabbitURL)
	require.NoError(t, err)
	t.Cleanup(func() { consumer.Close() })

	pub, err := NewRabbitPublisher(testRabbitURL)
	require.NoError(t, err)
...
func TestNewRabbitPublisher_BadURL(t *testing.T) {
	_, err := NewRabbitPublisher("amqp://bad:bad@localhost:1/")
	assert.Error(t, err)
}

func TestNewRabbitConsumer_BadURL(t *testing.T) {
	_, err := NewRabbitConsumer("amqp://bad:bad@localhost:1/")
	assert.Error(t, err)
}
